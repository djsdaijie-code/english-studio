...
验收程序只读取凭据是否存在，不输出 Key。当前机器检测结果：MiniMax 凭据未配置、Azure Speech 凭据未配置，因此没有发起网络请求，也没有伪造 Provider 成功、缓存命中或云评分结果。

本地设备验收使用 `scripts/verify_audio_devices.py --playback --record` 各执行一次短测试：检测到 1 个音频输入和 5 个音频输出，约 0.25 秒低音量提示音进入 playing 状态，约 0.8 秒临时录音成功生成；无播放或录音错误，录音未上传并在检查后删除。
...
课程页直接朗读没有独立播放控件时也会显示错误；失败的 listening/speaking 活动不会停在虚假的进行中或完成状态。日志记录稳定键、provider、模型、字符数和错误分类，不记录 Key、课程正文或录音内容。

### MiniMax 人工验收

1. 在设置中配置有效 MiniMax Key，使用隔离数据目录启动测试构建。
2. 打开课程 Day 1，只选一条短句朗读一次，确认播放；再次播放确认不发起新请求。
