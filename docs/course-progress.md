...
Phase 3 在 Phase 2 只读课程加载器之上增加最小用户状态层。课程正文、层级、顺序和必做规则仍以 `courses/` 下经过校验的 JSON 为权威源；SQLite 只保存 enrollment 和实际发生过的 Item 状态，不复制英文、中文、活动列表或课程层级。
...
- `CourseProgressRepository`：读写 schema 12 的稀疏用户状态。
- `CourseEnrollment`、`CourseItemProgress`、`CourseProgressSummary`：对外返回的冻结 dataclass，不暴露可变字典或 SQLite 内部 ID。
...
- 新增必做 Item 不预写状态，并会进入当前分母，所以完成率可以下降。
...
迁移从 schema 11 顺序执行到 12。迁移运行在 savepoint 内，失败会回滚表和版本号；新数据库可直接初始化到 12，重复启动不会重复建表或写 enrollment。

## enrollment 生命周期

显式 `enroll()` 是幂等的。第一次 start、complete 或 skip Item 时也会自动 enrollment。纯查询不会 enrollment，也不会预生成所有 Item 行。

暂停和归档只改变状态，不删除历史。`active` enrollment 完成当前全部有效必做 Item 后自动变为 `completed`。后续内容版本新增必做 Item 时，动态进度会重新变为未完成；用户开始新 Item 时，已完成 enrollment 会重新激活。暂停或归档状态不会被学习写入自动覆盖。
...
```text
完成率 = 当前已 completed 的有效必做 Item / 当前有效必做 Item
```

- 只有 required activity 引用的 Item 进入分母；可选 Item 不进入。
- `deprecated` Unit、Lesson 和 Item 不进入。
- `skipped` 保留为历史选择，但不视为完成。
- 同一 stable key 在一个聚合范围内只计一次。
- Lesson、Unit、Course 的百分比都实时聚合，不落库。
- 空 Lesson 明确定义为 `0 / 0`、`0%` 且未完成；它不会阻挡下一条可学习必做 Item 的推荐。
- 分数只记录，不构成 Phase 3 的完成门槛。

## 下一课与下一必做项

推荐顺序来自当前只读课程对象的 Level、Unit、Lesson 和 Sentence 排序。服务选择第一个既未 `completed`、也未 `skipped` 的有效必做 Item；`get_next_lesson()` 返回包含该 Item 的 Lesson。跳过前置项不会把它标记完成，但推荐可继续向后移动。

新用户可查询首个推荐，而不会因此写 enrollment。课程当前必做项全部完成时返回 `None`。`paused` 或 `archived` enrollment 也返回 `None`；恢复为 `active` 后重新按当前 JSON 和历史 Item 状态计算。Phase 3 不实现 Level 强制解锁，调用方仍可进入任意 Lesson。
...
Phase 3 不创建 FSRS 卡，不改听写或跟读记录，不把课程句子插入文章/词汇表，也不创建课程内容映射表。当前目标只是稳定保存最小学习状态。FSRS、听写、跟读和课程 Item 的映射需要等具体学习流程确定后再单独设计，避免提前耦合现有文章与词汇生命周期。
