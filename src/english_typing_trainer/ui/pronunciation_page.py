...
class PronunciationPage(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        super().__init__(parent); self.entry=None; self.context=None; self.audio_path:Path|None=None; self._build()

    def _build(self) -> None:
        root=QVBoxLayout(self); root.setContentsMargins(32,24,32,24); root.setSpacing(16)
        top=QHBoxLayout(); back=QPushButton("返回单词本"); back.setProperty("variant","ghost"); back.clicked.connect(self.back_requested.emit); self.title=QLabel("跟读评分 Beta"); self.title.setProperty("role","page-title"); top.addWidget(back); top.addWidget(self.title,1); root.addLayout(top)
    ...
    def load_target(self, entry: VocabularyEntry, context: VocabularyContext | None) -> None:
        self.entry=entry; self.context=context; self.audio_path=None; self.status.setText("准备开始跟读。"); self.scores.setText("完成 Azure 评分后将在这里显示准确度、流利度、完整度和韵律反馈。"); self.feedback.clear(); self._refresh_reference()
