...
第一版规划 5 个 Level、8 个 Unit、56 个学习日、约 192 个新句子。每个标准 Unit 约 7 天、24 个新句子：Day 1–4 各 6 句，Day 5 综合场景，Day 6 听写与跟读，Day 7 复习与测评。实际创作可在内容审核后调整；参数写入课程或 Unit 数据，不写死在程序与 Schema 中。

当前 `unit-01-foundations.json` 只有两天、12 句，用于验证结构，不代表 Unit 1 已完成。

## 难度递进

1. Level 1 以 4–8 词的直接命令和简单界面陈述为主。
2. Level 2 引入任务动词、比较、格式和输出限制，句子逐步达到 6–12 词。
3. Level 3 引入文件上下文、失败原因、条件和修正步骤，要求跨句理解。
4. Level 4 使用 API、认证、参数和限制等技术概念，兼顾短术语句与说明句。
5. Level 5 使用条件、权限、确认、重试和监控描述多步骤工作流。
